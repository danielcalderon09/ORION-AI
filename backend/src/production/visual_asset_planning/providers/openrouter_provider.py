"""OpenRouter visual asset planning adapter over the shared neutral transport."""
...
class OpenRouterVisualAssetPlanningProvider:
    def __init__(
        self,
        *,
        api_key: str,
        ...
        if not api_key.strip() or not model.strip():
            raise VisualAssetPlanningProviderConfigurationException(
                "visual asset planning provider credential or model is missing"
            )
        if not 1 <= max_transport_attempts <= 5:
            raise VisualAssetPlanningProviderConfigurationException(
                "visual asset planning transport attempts must be between 1 and 5"
            )
        if timeout_seconds <= 0 or retry_base_delay_seconds <= 0:
            raise VisualAssetPlanningProviderConfigurationException(
                "visual asset planning timeout and retry delay must be positive"
            )
        if max_output_tokens < 1 or not 0 <= temperature <= 2:
            raise VisualAssetPlanningProviderConfigurationException(
                "visual asset planning output token or temperature setting is invalid"
            )
        try:
            ...
            raise VisualAssetPlanningProviderConfigurationException(
                "visual asset planning provider HTTP configuration is invalid"
            ) from exc
    ...
    async def generate_visual_asset_plan(
        ...
        try:
            ...
            raise VisualAssetPlanningProviderContractException(
                "visual asset planning prompt could not be constructed"
            ) from exc
        payload = {
            "model": self._model,
            "messages": [
            ...
            "response_format": {
                "type": "json_schema",
                "json_schema": {
                    "name": "production_visual_asset_plan",
                    "strict": True,
        ...
        try:
            ...
            raise VisualAssetPlanningProviderAuthenticationException(
                "visual asset planning provider rejected authentication"
            ) from exc
        except OpenAICompatibleRateLimitError as exc:
            raise VisualAssetPlanningProviderRateLimitException(
                "visual asset planning provider rate limit reached"
            ) from exc
        except OpenAICompatibleTimeoutError as exc:
            raise VisualAssetPlanningProviderTimeoutException(
                "visual asset planning request timed out"
            ) from exc
        except OpenAICompatibleUnavailableError as exc:
            raise VisualAssetPlanningProviderUnavailableException(
                "visual asset planning provider is unavailable"
            ) from exc
        except OpenAICompatibleProtocolError as exc:
            if exc.status_code == 400:
                raise VisualAssetPlanningStructuredOutputException(
                    "selected provider model does not support required Structured Outputs"
                ) from exc
            raise VisualAssetPlanningProviderResponseException(
                "visual asset planning provider returned an invalid response"
            ) from exc
        latency_ms = max(0.0, (self._monotonic() - started) * 1000)
        try:
            plan = ProductionVisualAssetPlan.model_validate(load_strict_json_object(text))
            validate_visual_asset_plan_against_scene_plan(plan, request.scene_plan)
        except (ValueError, ValidationError, TypeError) as exc:
            raise VisualAssetPlanningProviderContractException(
                "visual asset planning provider output failed contract validation"
            ) from exc
