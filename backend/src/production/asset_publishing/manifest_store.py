...
def published_asset_manifest_relative_path(*, job_id: UUID, attempt_number: int) -> str:
    if attempt_number < 1:
...
class InMemoryPublishedAssetManifestStore:
    ...
    async def read(self, *, job_id: UUID, attempt_number: int) -> PublishedAssetManifest | None:
    ...
    async def create(self, manifest: PublishedAssetManifest) -> None:
        ...
        if key in self.manifests:
            raise PublishedAssetManifestConflictError("published manifest already exists")
    ...
    async def checkpoint(
        ...
        *,
        ...
        if self.manifests.get(key) != previous:
            raise PublishedAssetManifestConflictError("published manifest changed concurrently")
...
class LocalPublishedAssetManifestStore:
    ...
    def _read_sync(self, job_id: UUID, attempt_number: int) -> PublishedAssetManifest | None:
    ...
    def _list_sync(self) -> tuple[PublishedAssetManifest, ...]:
        ...
        for job_directory in sorted(production.iterdir(), key=lambda item: item.name):
            ...
            for attempt_directory in sorted(publishing.iterdir(), key=lambda item: item.name):
                ...
                if target.exists() or target.is_symlink():
                    ...
                    if manifest.job_id != job_id or manifest.attempt_number != int(match.group(1)):
    ...
    def _read_file(self, target: Path) -> PublishedAssetManifest:
        try:
            ...
            if not content or len(content) > self._maximum:
                raise PublishedAssetManifestCorruptError("published manifest size is invalid")
            return deserialize_published_asset_manifest(content)
        except PublishedAssetManifestError:
            raise
        except (OSError, UnicodeError, ValueError, TypeError) as exc:
            raise PublishedAssetManifestCorruptError("published manifest is invalid") from exc
    ...
    def _replace(self, target: Path, content: bytes) -> None:
        ...
        try:
            ...
            raise PublishedAssetManifestError("published manifest could not be persisted") from exc
        finally:
            if descriptor >= 0:
                os.close(descriptor)
    ...
    def _lock(self, target: Path) -> Iterator[None]:
        ...
        try:
            ...
            raise PublishedAssetManifestConflictError("published manifest is locked") from exc
        try:
            os.close(descriptor)
            yield
        finally:
