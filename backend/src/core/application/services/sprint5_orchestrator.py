...
class Sprint5Orchestrator:
    ...
    async def process_video(
        ...
        try:
            ...
            stage_index += 1
            vu = await self._run_with_context(
                "video_understanding",
                project,
                {
            ...
            stage_index += 1
            attention = await self._run_with_context(
                "attention",
                project,
                {
                    "vision_features": vision.features,
            ...
            stage_index += 1
            narrative = await self._run_with_context(
                "narrative",
                project,
                {
                    "vision_features": vision.features,
            ...
            viral = await self._run_with_context(
                "viral_score",
                project,
                {
                    "vision_features": vision.features,
            ...
            stage_index += 1
            audience = await self._run_with_context(
                "audience_director",
                project,
                {
                    "content_features": {
            ...
            hooks = await self._run_with_context(
                "hook_optimizer",
                project,
                {
                    "selected_clips": [],
            ...
            stage_index += 1
            retention = await self._run_with_context(
                "retention_simulator",
                project,
                {
                    "selected_clips": [],
            ...
            stage_index += 1
            creative = await self._run_with_context(
                "creative_director",
                project,
                {
                    "attention_features": attention.features,
            ...
            candidates = await self._run_with_context(
                "candidate_generator",
                project,
                {
            ...
            for cs in candidate_sets:
                for candidate in cs.get("candidates", []):
                    # Critic
                    critique = await self.agents["critic_ai"].execute(
                        AgentInput(
                            ...
                            context={
                                ...
                                "context": {
                                    ...
                                    "retention_curve": next(
                                        (
                                            r
                                            for r in retention.features.get("retention_curves", [])
                                            if r["clip_id"].startswith(
                                                f"clip_{candidate['start']:.1f}"
                                            )
                                        ),
                                        {},
                                    ),
                    ...
                    # Reflection
                    reflection = await self.agents["reflection_engine"].execute(
                        AgentInput(
                            media_reference=str(video_path),
                            context={
                                "clip": candidate,
                                ...
                                "metrics": {
                                    "quality_score": 0.7,
                                    "viral_score": candidate.get("estimated_viral_score", 0),
                                },
                                "retention_curve": next(
                                    (
                                        r
                                        for r in retention.features.get("retention_curves", [])
                                        if r["clip_id"].startswith(f"clip_{candidate['start']:.1f}")
                                    ),
                                    {},
                                ),
                            },
            ...
            consensus = await self._run_with_context(
                "consensus_engine",
                project,
                {
            ...
            for cr in consensus_results:
                ...
                dop_input = AgentInput(
                    ...
                    context={
                        ...
                        "edit_decisions": [
                            {
                                "clip_id": winner["variant_id"],
                                "temporal_range": (winner["start"], winner["end"]),
                            }
                        ],
                    },
                )
                ...
                framing = (
                    dop_result.features.get("framed_decisions", [{}])[0].get("framing", {})
                    if dop_result.features.get("framed_decisions")
                    else {}
                )

                # Export
                exp_input = AgentInput(
                    media_reference=str(video_path),
                    context={
                ...
                # QA
                qa_input = AgentInput(
                    media_reference=exp_result.features["output_path"],
                    context={
                        "expected_params": {
                ...
                pattern = {
                    ...
                    "action": {
                        "start": winner["start"],
                        "end": winner["end"],
                        "strategy": winner.get("hook_strategy"),
                    },
                    "outcome": {},
                    "usage_count": 1,
                    "success_rate": 0.5,
            ...
            self.observability.emit_event(
                "pipeline.completed",
                {
                    ...
                    "platform": platform,
                    "profile_name": profile_name,
                    "total_duration_seconds": profile_summary.get("total_time_sec")
                    if profile_summary
                    else None,
                    "peak_memory_mb": profile_summary.get("peak_memory_mb")
                    if profile_summary
                    else None,
            ...
            self.benchmark.record_run(
                ...
                output_metrics={
                    ...
                    "candidates_evaluated": sum(
                        len(cs.get("candidates", [])) for cs in candidate_sets
                    ),
            ...
            await self.event_bus.emit(
                "pipeline.failed", {"project_id": str(project.project_id), "error": str(e)}
            )
    ...
    async def _run(self, agent_name, project, video_path, stages, stage_index):
        ...
        if cached is not None:
            self.observability.emit_event(
                "pipeline.cache.hit", {"project_id": str(project.project_id), "stage": agent_name}
            )
            return cached
        ...
        self.memory_manager.request_allocation(
            str(project.project_id), agent_name, self.memory_manager.budget.max_ram_mb * 0.15
        )

        result = await self.agents[agent_name].execute(AgentInput(media_reference=str(video_path)))

        self.feature_store.save(
            project.project_id, f"{agent_name}_agent", f"{agent_name}_features", result.features
        )
        ...
        self.observability.emit_event(
            ...
            {"project_id": str(project.project_id), "stage": agent_name, "duration": duration},
        )
        self.checkpoint_manager.save_checkpoint(
            project.project_id,
            f"{agent_name.upper()}_DONE",
            stage_index,
            brain_state={},
            feature_paths={},
        )
    ...
    async def _run_with_context(self, agent_name, project, context, stages, stage_index):
        ...
        result = await self.agents[agent_name].execute(
            AgentInput(media_reference=str(project.source_path), context=context)
        )
